<template>
  <div
    :class="['cms-content py-10 page-content', { 'container': sync }]"
    v-if="data"
    v-html="data.content"
  />
</template>
...
export default {
  ...
  created () {
    ...
    if (queryKey && queryValue) {
      this.$store.dispatch('cmsPage/single', {
        key: queryKey,
        value: queryValue,
        skipCache: this.sync
      })
  ...
  computed: {
    data () {
      if (this.id) {
        return this.$store.getters[`cmsPage/cmsPageId`](this.id)
      } else if (this.identifier) {
        return this.$store.getters[`cmsPage/cmsPageIdentifier`](this.identifier)
      }
    }
  }
}
