...
export default {
  name: 'LegacyProductfromSearchSpring',
  data () {
    return {
      currentPage: 0,
      sliderConfig: {
  ...
  async beforeMount () {
      ...
      if(this.trendingSearches.length){
        this.randomitem = this.trendingSearches[Math.floor(Math.random() * this.trendingSearches.length)];
  ...
  methods: {    
    async getDataFromElastic (searchedData, onScroll = false ,  searchType) {      
      ...
      query = query.applyFilter({ key: 'sku.keyword', value: { eq: searchedData } });
