<template>
  ...
  <div class="customer-address-form">
    <div class="row gutter-md">
      ...
      <div class="col-6 col-xs-6 col-md-6" v-if="hideDefalutAddressChkBox">
        <base-checkbox
          id="default_shipping"
      ...
      <div class="col-12 col-xs-12 col-sm-6 col-12 mt-5">
        <button-full class="flex" style="float: left;"
...
export default {
  ...
  computed: {
    ...
    hideDefalutAddressChkBox () {
        let showFlag = true;
        const selectedData = this.$store.state.user.current.addresses.find(val=> toString(val.id) === toString(this.addressId))
        if (selectedData && selectedData.default_billing)  showFlag = false;
        if (selectedData && selectedData.default_shipping) showFlag = false;
        return showFlag
    }
  },
  data () {
  ...
  methods: {
    ...
    addcustomerAddress () {
      if (this.addressId === null) {
        this.addNewAddress(this.customer)
      } else {
        if (!this.objectsEqual(this.customer, this.getCustomerAddress())) {
          this.updateEditedAddress(this.addressId, this.customer)
        }
      }
      // this.exitSection()
    },
