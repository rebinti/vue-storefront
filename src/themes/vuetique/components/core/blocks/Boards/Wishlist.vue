<template>
  <!-- <div class="wishlist wishlist_out right-sidebar max-w-full fixed p-8 pt-10" :class="{ active: isWishlistOpen }" style="text-align: center;">
    ...
    <div  class="wish_wrap_box" :class="{'item-in-it': boardsListItems.length>0}">
        ...
        <!-- Boards Listing -->
        <h4 v-if="!boardsListItems.length" class="mb-2">
          {{ $t('Your Board is empty.') }}
        </h4>
        ...
        <h2 v-if="isBoardsItemsOpen" class="mb-8 upper-letter" style="height: 60px;background: #f5f2f1;padding-top: 8px;">
            <span @click="changeBoardView();$emit('chagesInView')" style="float: left;padding-left: 17px;"> 
              <i class="fa fa-chevron-left" aria-hidden="true"></i> </span>  
            &nbsp; &nbsp; &nbsp; 
            {{ selectedBoardItem.name }}  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <span v-if="selectedBoardItem && selectedBoardItem.items && selectedBoardItem.items.length > 0" style="font-size: 14px;" >
              {{ selectedBoardItem.items.length }}</span> 
            <span style="font-size: 14px;" v-else>0</span> <span style="font-size: 11px;text-transform: lowercase;">Items</span> 
        </h2>
      ...
      <!-- Boards products Listing Empty screen -->
        <p v-if="selectedBoardItem && selectedBoardItem.items && selectedBoardItem.items.length == 0
        && isBoardsItemsOpen"  class="mb-2">
          {{ $t('Boards is empty.') }}
        </p>
    </div>
  <!-- </div> -->
