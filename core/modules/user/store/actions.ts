...
const actions: ActionTree<UserState, RootState> = {
   ...
   */
  resetPasswordPost (context, { password, confirmation, id, token }) {
    return TaskQueue.execute({ url: config.users.resetPasswordPost_endpoint,
      payload: {
        ...
        body: JSON.stringify({ password: password, confirmation:confirmation, id:id, token:token })
      }
    })
  },  
  /**
   ...
   * Login user and return user profile and current token
   */
  async register (context, { email, firstname, lastname, password , mobile }) {
    ...
    return fetch(processURLAddress(url), { method: 'POST',
      ...
      body: JSON.stringify({ customer: { email: email, firstname: firstname, lastname: lastname, mobile: mobile }, password: password })
    }).then(resp => { return resp.json() })
  },
  ...
  async update (context, userData: UserProfile) { //  userData: UserProfile)
    if (userData.customer.addresses && userData.customer.addresses.length > 0) {
        userData.customer.addresses.map((val) => {
          ...
          if (val.company) val.company = val.company ? val.company.toString() : 'Nil';
          if (val.company == null) val.company = 'Nil';
